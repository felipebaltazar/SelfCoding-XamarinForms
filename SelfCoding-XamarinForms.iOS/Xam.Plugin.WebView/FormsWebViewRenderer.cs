...
namespace Xam.Plugin.WebView.iOS
{
    public class FormsWebViewRenderer : ViewRenderer<FormsWebView, WKWebView>, IWKScriptMessageHandler, IWKUIDelegate
    {
		...
		void SetupElement(FormsWebView element)
		{
            ...
            element.OnJavascriptInjection += OnJavascriptInjectionRequest;
        ...
        private readonly ConcurrentQueue<string> _jsQueue = new ConcurrentQueue<string>();
        internal async Task<string> OnJavascriptInjectionRequest(string js)
		{
            _jsQueue.Enqueue(js);
            if (Control == null || Element == null) return string.Empty;

            if (Control.Superview is null || Control.Window is null || Control.IsLoading)
                return string.Empty;
            ...
            try
            {
                while (_jsQueue.TryDequeue(out var jsToExecute) && !Control.IsLoading)
                {
                    
                    var obj = await Control.EvaluateJavaScriptAsync(jsToExecute);
                    if (obj != null)
                        response = obj.ToString();
                }
            }

            catch (Exception e) { System.Diagnostics.Debug.WriteLine(e.Message); /* The Webview might not be ready... */ }
            return response;
		}
        ...
        {
            if (Element != null)
                Element.CurrentUrl = Control.Url.AbsoluteString;

            if (_jsQueue.TryDequeue(out var waitingJs))
                OnJavascriptInjectionRequest(waitingJs).SafeFireAndForget();
        }
